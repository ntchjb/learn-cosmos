...
# docker buildx build --tag learncosmos --file .pi/Dockerfile --platform linux/arm64 --cache-from learncosmos:cache --cache-to learncosmos:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar learncosmos
...
sudo bash -c "echo learncosmos > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/learncosmos.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/learncosmos.img
